...
class LipsyncPipeline(DiffusionPipeline):
    ...
    def affine_transform_video(self, video_path):
        ...
        print(f"面部对齐总时间: {total_time:.4f}秒")
        print(f"处理帧数: {len(video_frames)}")
        print(f"平均每帧处理时间: {avg_frame_time:.4f}秒")
        
        # 生成详细的性能统计报告
        if len(processing_times) > 1:
            performance_report = self.analyze_performance_stats(processing_times)
            print("\n==== 面部对齐性能详细报告 ====")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # timesteps = torch.tensor([999, 666, 333], device=device)
        ...
        for i in tqdm.tqdm(range(num_inferences), desc="Doing inference..."):
            ...
            if self.unet.add_audio_layer:
                ...
                audio_embeds = None
            print(f"Audio embeddings preparation took: {time.time() - audio_time_start:.4f}s")
            
            # Face and mask preparation
            ...
            print(f"Face and mask preparation took: {time.time() - face_time_start:.4f}s")

            # Mask latents preparation
            ...
            print(f"Mask latents preparation took: {time.time() - mask_time_start:.4f}s")

            # Image latents preparation
            img_latents_time_start = time.time()
            ...
            print(f"Image latents preparation took: {time.time() - img_latents_time_start:.4f}s")
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for j, t in enumerate(timesteps):
                    ...
                    print(f"Denoising step {j} took: {time.time() - step_time_start:.4f}s")
            print(f"Total denoising loop took: {time.time() - denoising_time_start:.4f}s")
            ...
            print(f"Decoding and post-processing took: {time.time() - decode_time_start:.4f}s")
            
            print(f"Total iteration {i} took: {time.time() - start_time:.4f}s\n")
    ...
    def analyze_performance_stats(self, processing_times):
        """
        分析性能统计数据并生成汇总报告
        
        Args:
            processing_times: 包含处理时间的列表或字典
        ...
        if isinstance(processing_times, list):
            ...
            report = "性能统计汇总:\n"
            report += f"总样本数: {len(times)}\n"
            report += f"平均处理时间: {np.mean(times):.4f}秒\n"
            report += f"最小处理时间: {np.min(times):.4f}秒\n"
            report += f"最大处理时间: {np.max(times):.4f}秒\n"
            report += f"标准差: {np.std(times):.4f}秒\n"
            report += f"中位数: {np.median(times):.4f}秒\n"
            
            # 百分位数统计
            percentiles = [25, 50, 75, 90, 95, 99]
            for p in percentiles:
                report += f"{p}百分位数: {np.percentile(times, p):.4f}秒\n"
                
            return report
        else:
            return "提供的数据格式不正确，无法生成报告。"
